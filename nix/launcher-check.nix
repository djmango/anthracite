...
{ pkgs }:
let
  supervisor = pkgs.writeScript "anthracite-supervisor-probe" ''
    #!${pkgs.nushell}/bin/nu --no-config-file
    ${builtins.readFile ../tests/supervisor-probe.nu}
  '';
...
in pkgs.runCommand "anthracite-launcher-check" { nativeBuildInputs = [ pkgs.nushell ]; } ''
  nu --no-config-file ${../tests/launcher.nu} ${../devutils/launch.nu} ${probe} ${supervisor}
  touch "$out"
''
