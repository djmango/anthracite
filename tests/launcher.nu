...
def main [launcher: path, probe: path, supervisor: path] {
    use std/assert
    ...
    let first = ^nu --no-config-file $launcher $probe --supervisor $supervisor 'model with spaces.FCStd' | complete
    ...
    let second = ^nu --no-config-file $launcher $probe --supervisor $supervisor 'model with spaces.FCStd' | complete
    assert equal $second.exit_code 0 $second.stderr
    assert equal (open --raw $preferences) 'user layout'
    let invalid = with-env {XDG_CONFIG_HOME: relative} { ^nu --no-config-file $launcher $probe --supervisor $supervisor | complete }
